...
##------------------------------------------------------------------------------
ReManualRTvalidator <- function(AutomatedRtExtractorList, ReManualRTvalidatorID, Intensity2 = 1000){
  library(patchwork)
  library(ggplot2)
  library(ggplotify)
  library(ggbump)
  ...
  ## 循环list的指定元素
  for (ith in ReManualRTvalidatorID){
    ## 获取Addcut信息
    ithAdductList <- AutomatedRtExtractorList[[ith]]$Addcuts
    ## 循环每一个Addcut
    for (jth in which(names(ithAdductList) == "M+H" | names(ithAdductList) == "M-H" | names(ithAdductList) == "M+" | names(ithAdductList) == "M++" | names(ithAdductList) == "M-")){
      ## 获取第j个Addcut的信息
